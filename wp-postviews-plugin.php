...
class PostViews
{
    ...
    public function show_views(){
        ...
        <div class="misc-pub-section edit-post-author">
            <span class="dashicons dashicons-visibility"></span> <?= sprintf( _n( '%s view', '%s views', $views, 'views' ), $views ) ?>
        </div>
        <?php
    }
    ...
    protected function views_filter(){
        ...
        if( !($post instanceof WP_Post) ||
            ...
            !empty(array_intersect($this->settings['no_count_for'], wp_get_current_user()->roles))
        ){
            $allow = false;
        }
    ...
    {
        wp_enqueue_script($this->settings['localize']);
        wp_localize_script(
